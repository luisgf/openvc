...
| Swap / replay a status list or presentation | Stale-status / replay accept | Status-list token `sub` must equal the fetched URI (anti-swap); presentations bind `aud` + one-time `nonce`/`challenge`; a fetched status list is **verified** before it is trusted. On SD-JWT VC with `require_key_binding=True`, the KB-JWT is verified even when the issuer JWT is expired, so an `EXPIRED` verdict is not a substitute for holder binding |

**[openvc](Home)**

**Start**

- [Getting Started](Getting-Started)

**Proof formats**

- [VC-JWT](VC-JWT)
- [SD-JWT VC](SD-JWT-VC)
- [Data Integrity](Data-Integrity)

**Verifying in practice**

- [Presentations & OpenID4VP](Presentations)
- [Resolving issuer keys](Resolving-Issuer-Keys)
- [Relying-party certificates](Relying-Party-Certificates)
- [Credential schemas](Credential-Schemas)
- [Status lists](Status-Lists)
- [Trust anchors: EU TL & EBSI](Trust)
- [Async verification](Async-Verification)

**Operating**

- [Keys & HSM backends](Keys-and-HSM)
- [Security model](Security-Model)
- [Versioning & deprecation](Versioning-and-Deprecation)

**Reference**

- [API reference ↗](https://luisgf.github.io/openvc/)
- [Examples ↗](https://github.com/luisgf/openvc/tree/main/examples)
- [Changelog ↗](https://github.com/luisgf/openvc/blob/main/CHANGELOG.md)

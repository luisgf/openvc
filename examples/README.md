...
| `13_oid4vci_key_attestation.py` | OpenID4VCI **attested-key** proofs: the signing key is inside the header's `key_attestation` (App. D), so the issuer resolves it through `resolve_proof_key_in_context` with its own `kid`→`attested_keys` rule, reads the attestation with `peek_key_attestation` — unverified, its trust is the issuer's call — and watches a key the attestation does not carry get refused |

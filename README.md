...
| **Verify presentations** | VP-JWT, Data Integrity `challenge`/`domain`, and stateless [OpenID4VP 1.0](https://openid.net/specs/openid-4-verifiable-presentations-1_0.html) `vp_token` — incl. HAIP `direct_post.jwt` JWE-encrypted responses and the **W3C Digital Credentials API** (origin-bound) | OpenID4VP / HAIP / DC API |
| | EUDI relying-party access certificate (WRPAC) — the X.509 identity of the *requester*, validated to ACA anchors | [ETSI TS 119 411-8](https://www.etsi.org/standards) |
...
| **Issuer side** | [OpenID4VCI 1.0](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0.html) **key-proof verification** — verify the `openid4vci-proof+jwt` a wallet sends to your Credential Endpoint and get back the key to bind the credential to. Stateless: no endpoint, no Authorization Server, nonce single-use injected as a required callable | OpenID4VCI |
...
- **[`examples/`](https://github.com/luisgf/openvc/blob/main/examples/)** —
  thirteen runnable, offline scripts covering every flow (they run in CI, so they
  cannot rot).
